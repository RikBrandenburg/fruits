                    ...
                    <form method="post" action="{{ route('fruit.update', $fruit->id) }}">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" id="name" name="name"
                                ...
                                value="{{ old('name', $fruit->name) }}" required autofocus />
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" id="colour" name="colour"
                                class="form-input rounded-md shadow-sm mt-1 block w-full"
                                value="{{ old('colour', $fruit->colour) }}" required />
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" id="weight" name="weight"
                                class="form-input rounded-md shadow-sm mt-1 block w-full"
                                value="{{ old('weight', $fruit->weight) }}" required />
                        </div>
                            ...
                            <select name="family_id" id="family_id"
                                ...
                                @foreach ($families as $family)
                                    <option value="{{ $family->id }}"
                                        {{ old('family_id', $fruit->family_id) == $family->id ? 'selected' : '' }}>
                                        {{ $family->name }}
                                    </option>
                                @endforeach
                            </select>
